...
const CustomTooltip = ({ active, payload }) => {
    ...
    return (
          <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#f8fafc", boxShadow: "0 4px 24px rgba(0,0,0,0.4)" }}>
                  <div style={{ fontWeight: 700, marginBottom: 4 }}>{d.name}</div>div>
                  <div style={{ color: "#94a3b8" }}>{fmt(d.value)}</div>div>
                  <div style={{ color: "#94a3b8" }}>{fmtPct(d.pct)}</div>div>
          </div>div>
        );
};
        ...
        return (
                <div style={{ ...S.page, ...S.centre, background: "#080e1a" }}>
                          <div style={{ maxWidth: 580, width: "100%" }}>
                                      <div style={S.label}>One-time setup</div>div>
                                      ...
                                      <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.7, marginBottom: 28 }}>
                                                    You'll need a Google Cloud OAuth Client ID. This takes about 5 minutes and means your sheet stays completely private.
                                      </p>p>
                            {[
                  { n: 1, text: <>Go to <span style={{ color: "#60a5fa" }}>console.cloud.google.com</span>span> &rarr; Create a new project (or select existing)</>> },
                  { n: 2, text: <>APIs &amp; Services &rarr; Enable APIs &rarr; search <span style={{ color: "#60a5fa" }}>Google Sheets API</span>span> &rarr; Enable</>> },
                  { n: 3, text: <>APIs &amp; Services &rarr; Credentials &rarr; Create Credentials &rarr; <span style={{ color: "#60a5fa" }}>OAuth Client ID</span>span></>> },
                  { n: 4, text: <>Application type: <span style={{ color: "#60a5fa" }}>Web application</span>span>. Under Authorised JavaScript origins add <span style={{ color: "#60a5fa" }}>{window.location.origin}</span>span></>> },
                  { n: 5, text: <>Copy the <span style={{ color: "#60a5fa" }}>Client ID</span>span> and paste it below</>> },
                            ].map(({ n, text }) => (
                                          <div key={n} style={S.step}>
                                                        <div style={S.stepNum}>{n}</div>div>
                                                        <div style={{ fontSize: 13, color: "#64748b", lineHeight: 1.6, paddingTop: 3 }}>{text}</div>div>
                                          </div>div>
                                        ))}
                                    <div style={{ marginTop: 24 }}>
                                                <div style={S.label}>Your OAuth Client ID</div>div>
                                                <input value={clientId} onChange={e => setClientId(e.target.value)}
                                                                placeholder="xxxxxxxxxxxx-xxxxxxxxxxxxxxxx.apps.googleusercontent.com" style={S.input} />
                                    </div>div>
                            {error && <div style={S.err}>{error}</div>div>}
                                    <button onClick={() => { if (!clientId.trim()) { setError("Please enter your Client ID."); return; } setError(""); sessionStorage.setItem("portfolio_clientId", clientId.trim()); setScreen(SCREEN.AUTH); }}
                                                  style={S.btn()}>Continue &rarr;</button>button>
                          </div>div>
                </div>div>
              );
  }
          ...
          return (
                  <div style={{ ...S.page, ...S.centre, background: "#080e1a" }}>
                          <div style={{ maxWidth: 460, width: "100%", textAlign: "center" }}>
                                    <div style={S.label}>Authentication</div>div>
                            ...
                            {error && <div style={{ ...S.err, textAlign: "left" }}>{error}</div>div>}
                                    <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
                                                <button onClick={() => setScreen(SCREEN.SETUP)} style={S.ghost}>&larr; Back</button>button>
                                                <button onClick={signIn} style={S.btn()}>Sign in with Google &rarr;</button>button>
                                    </div>div>
                          </div>div>
                  </div>div>
                );
                          ...
                          <div style={{ maxWidth: 500, width: "100%" }}>
                            {userEmail && (
                                <div style={{ fontSize: 11, fontFamily: "monospace", color: "#334155", marginBottom: 20 }}>
                                    ...
                                    <div style={S.label}>Load your sheet</div>div>
                                    <h1 style={{ fontSize: 26, fontWeight: 400, color: "#f8fafc", letterSpacing: "-0.02em", margin: "0 0 12px" }}>
                                    ...
                                    <div style={S.label}>Google Sheets URL</div>div>
                                    <input value={sheetUrl} onChange={e => setSheetUrl(e.target.value)}
                                                  placeholder="https://docs.google.com/spreadsheets/d/..." style={S.input}
                                                  onKeyDown={e => e.key === "Enter" && loadSheet()} />
                            {error && <div style={S.err}>{error}</div>div>}
                                    <div style={{ display: "flex", gap: 12 }}>
                                                <button onClick={() => setScreen(SCREEN.AUTH)} style={S.ghost}>&larr; Back</button>button>
                                                <button onClick={() => { if (sheetUrl) sessionStorage.setItem("portfolio_sheetUrl", sheetUrl); loadSheet(); }}
                                                                disabled={!sheetUrl || loading} style={{ ...S.btn(), opacity: !sheetUrl ? 0.4 : 1 }}>
    ...
    return (
          <div style={S.page}>
                <div style={{ marginBottom: 40, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                        <div>
                                  <div style={{ ...S.label, marginBottom: 8 }}>Live Portfolio &mdash; {fmt(total)} Investable</div>div>
                                  <h1 style={{ fontSize: 28, fontWeight: 400, margin: 0, color: "#f8fafc", letterSpacing: "-0.02em" }}>
                                              Current <span style={{ color: "#334155" }}>&rarr;</span>span> Target Allocation
                                  </h1>h1>
                        </div>div>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          {userEmail && <span style={{ fontSize: 11, fontFamily: "monospace", color: "#334155" }}>&#9679; {userEmail}</span>span>}
                                  <button onClick={() => { setScreen(SCREEN.SHEET); setData(null); }} style={S.ghost}>&#8634; Refresh</button>button>
                                  <button onClick={() => { setScreen(SCREEN.SETUP); setToken(null); setData(null); }} style={S.ghost}>Sign out</button>button>
                        </div>div>
                </div>div>
                  ...
                  {[{ label: "Current", data: current, subtitle: "Your portfolio today" },
                    { label: "Target", data: target, subtitle: "Suggested rebalance" }].map(({ label, data: d, subtitle }) => (
                                <div key={label} style={S.card}>
                                            <div style={S.label}>{label}</div>div>
                                            <div style={{ fontSize: 18, marginBottom: 20, color: "#cbd5e1" }}>{subtitle}</div>div>
                                            ...
                                            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 16px", marginTop: 16 }}>
                                              {d.map(e => (
                                                  <div key={e.name} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#64748b" }}>
                                                    ...
                                                    {e.name} <span style={{ color: "#94a3b8" }}>{fmtPct(e.pct)}</span>span>
                                                  </div>div>
                                                ))}
                                            </div>div>
                                </div>div>
                ...
                <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 28 }}>
                        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #1e293b" }}>
                                  <div style={S.label}>Allocation Changes</div>div>
                        </div>div>
                        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1.8fr", padding: "10px 24px", fontSize: 10, letterSpacing: "0.12em", color: "#334155", textTransform: "uppercase", fontFamily: "monospace", borderBottom: "1px solid #0f1d33" }}>
                                  <span>Asset</span>span><span style={{ textAlign: "right" }}>Current</span>span>
                                  <span style={{ textAlign: "right" }}>Target</span>span><span style={{ textAlign: "right" }}>Shift</span>span>
                                  <span style={{ textAlign: "right" }}>Movement</span>span>
                        </div>div>
                  {changes.map((row, i) => {
                      ...
                      return (
                                    <div key={row.name} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}
                                                  ...
                                                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                                                  ...
                                                                  <span style={{ fontSize: 14, color: "#cbd5e1" }}>{row.name}</span>span>
                                                  </div>div>
                                                  <div style={{ textAlign: "right", fontSize: 13, color: "#64748b", fontFamily: "monospace" }}>{fmtPct(row.from)}</div>div>
                                                  <div style={{ textAlign: "right", fontSize: 13, color: "#94a3b8", fontFamily: "monospace" }}>{fmtPct(row.to)}</div>div>
            ...
            { label: "Total Investable", get: () => [fmt(total), "Excl. primary residence", "#60a5fa"] },
                                ...
                                return (
                                              <div key={label} style={{ ...S.card, padding: "18px 20px" }}>
                                                            <div style={{ ...S.label, marginBottom: 8 }}>{label}</div>div>
                                                            <div style={{ fontSize: 22, color, fontFamily: "monospace", marginBottom: 4 }}>{value}</div>div>
                                                            <div style={{ fontSize: 12, color: "#475569" }}>{note}</div>div>
                                              </div>div>
                                            );
                    })}
                </div>div>
